...
var avgVal = 0, calcVal = 0, intervals = array(NUM_OF_CHANNELS), t2 = 0;
...
export function beforeRender(delta) {
  ...
  while (deltatime > smallstep) {
    ...
    sensitivity = calcPIController(pic, 0.5 - avgVal)

    frequencyData.forEach(function(x, i) {
      calcVal = clamp(pow(x * sensitivity, 2), 0, 1)
      if(maxFrequencyMagnitude == x) {
        calcVal = clamp(pow(x*sensitivity, 3), 0, 1)
      }
      if (calcVal > channels[i]) {
        channels[i] = calcVal;
        intervals[i] = 1;
      } else {
